    ...
    public List<TraverRecord> recordList(Double j, Double w, MemberInfo memberInfo, int type, int recordType, String cityCode, String eCityCode, Date time, int pageStart, int pageSize) {
        ...
        if (recordType == 3) {
            if (eCityCode == null) {
                ...
                return find(StringsKit.replaceSql(SqlManager.sql("traver.daihuo"), "and start_adcode=? and end_adcode = ?"), j, w, type, recordType, cityCode, eCityCode, Constant.TraverStatus.CREATE, time, pageStart, pageSize);
            }
        } else {
            if (recordType == 2) {
