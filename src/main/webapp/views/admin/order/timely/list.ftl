...
<!--suppress ALL -->
<input type="hidden" value="${(statusType)!}" id="statusType" xmlns="http://java.sun.com/jsf/html"/>
<ul class="nav nav-tabs">
    <#list statusMap.keySet() as key>
        <li  <#if statusType==key>class="active"</#if>>
            <a href="${ctx}/admin/order/goTimelyOrder?statusType=${key}">${statusMap.get(key)}</a>
        </li>
    </#list>
</ul>
                ...
                <table class="hover table table-striped table-bordered" cellspacing="0" width="200%">
                    <tr>
                        <td colspan="2">
                            ...
                            <input type="hidden" name="type" value="${(statusType)!}"/>
                ...
                <tr>
                    <td>
                        ...
                        <div class="countSum label label-primary"
                             data-url="${ctx}/admin/total/dingdanzonge?type=${(statusType)!}"
                             data-filter="false">
                            <span>总额：</span>
                            <img src="${ctx}/static/images/loading.gif" style="height: 20px;width: 20px;"/>
                            <span></span>
...
<script type="text/javascript" src="${ctx}/static/js/app/admin/timely_order.js?_${.now?string("hhmmss")}"></script>
