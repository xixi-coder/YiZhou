...
<ul class="nav nav-tabs">
    <#list serviceTypes as item>
        <li <#if type==item.id>class="active"</#if>>
            <a href="${ctx}/admin/schedule/order?type=${item.id}">${item.name}</a>
        </li>
    </#list>
</ul>
<div class="panel panel-default">
    <div class="panel-heading">
        <h4>专线订单管理</h4><div class="time"></div>
    ...
    <div class="btn-group" style="width:64px;">
        ...
        <ul class="dropdown-menu success btn-xs " role="menu">
            ...
            <#if hasPermission("zhuijiadingdan")>{{# if(d.status!=6&&d.status!=5&&d.status!=9){ }}
                ...
                {{#} }}</#if>
            <#if hasPermission("dispatchOrder")>{{# if(d.auto_dispatch_order==1&&d.status==1){ }}
                <li><a target="_blank" href="${ctx}/admin/order/dispatchOrder/{{d.id}}">派单</a></li>
                {{#} }}</#if>
        </ul>
    </div>
</script>
<script type="text/javascript" src="${ctx}/static/js/app/admin/zxorder.js?_${.now?string("hhmmss")}"></script>
