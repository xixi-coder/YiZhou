<#include "../base/_base.ftl">
<#macro title>
财务管理
</#macro>
<#macro content>
<!--suppress ALL -->
<input type="hidden" value="${(type)!}" id="orderType"/>
<ul class="nav nav-tabs" style="margin-top: 5px;margin-left: 5px;">
    <li class="tabs-select">
        <a href="${ctx}/admin/finance/1">财务明细</a>
    </li>
<#--<li class="tabs-select">-->
<#--<a href="${ctx}/admin/finance/2">财务明细(根据服务人员公司)</a>-->
<#--</li>-->
    <li class="tabs-select">
        <a href="${ctx}/admin/finance/depositlist/3">服务人员预存款</a>
    </li>
    <li class="tabs-select">
        <a href="${ctx}/admin/finance/depositlist/4">客户预存款</a>
    </li>
    <li class="tabs-select">
        <a href="${ctx}/admin/finance/loglist/5">服务人员充值记录</a>
    </li>
    <li class="tabs-select">
        <a href="${ctx}/admin/finance/loglist/6">客户账户明细</a>
    </li>
</ul>

<div class="panel-body">
    <div class="table-responsive" style="overflow:auto;">
        <form id="searchForm_">
            <table class="table table-bordered table-striped">
                <tr>
                    <td width="80">
                        <div class="form-group">
                            <label for="s-dod.no-LIKE" class="control-label">流水号:</label>
                            <input name="s-dod.no-LIKE"/>
                        </div>
                    </td>
                    <td width="80">
                        <div class="form-group">
                            <label for="s-dod.remark-LIKE" class="control-label">订单号:</label>
                            <input name="s-dod.remark-LIKE"/>
                        </div>
                    </td>
                    <#if hasRole("super_admin")||hasPermission("select-company")>
                        <td>
                            <label class="control-label">所属公司:</label>
                            <select class="select2 select-company" name="s-dc.id-EQ"
                                    style="width: 100px;">
                                <option value="">请选择</option>
                            </select>
                        </td>
                    </#if>
                    <td>
                        <label for="s-name-LIKE" class="control-label">时间:</label>
                        <input type="hidden" name="s-dod.create_time-BETWEEN" value="" id="timeSearch">
                        <input type="text" data-date-format="yyyy-MM-dd"
                               class="datepicker-input" id="start"
                               placeholder="如:1992-11-11">至
                        <input type="text" data-date-format="yyyy-MM-dd"
                               class="datepicker-input" id="end"
                               placeholder="如:1992-11-11">
                    </td>
                    <td>
                        <button type="button" class="btn btn-info search">搜索</button>
                    </td>
                </tr>
            </table>
        </form>
    </div>
    <div class="table-responsive" style="overflow:auto;height: 30px;">
        <table>
            <tr>
                <td>
                    <div class="countSum label label-primary" data-url="${ctx}/admin/total/zongshouru"
                         data-filter="false">
                        <span>总收入：</span>
                        <img src="${ctx}/static/images/loading.gif" style="height: 20px;width: 20px;"/>
                        <span></span>
                        <span>元</span>
                    </div>
                    <div class="countSum label label-primary" data-url="${ctx}/admin/total/yearshouru"
                         data-filter="false">
                        <span>本年收入：</span>
                        <img src="${ctx}/static/images/loading.gif" style="height: 20px;width: 20px;"/>
                        <span></span>
                        <span>元</span>
                    </div>
                    <div class="countSum label label-primary" data-url="${ctx}/admin/total/monuthshouru"
                         data-filter="false">
                        <span>本月收入：</span>
                        <img src="${ctx}/static/images/loading.gif" style="height: 20px;width: 20px;"/>
                        <span></span>
                        <span>元</span>
                    </div>
                    <div class="countSum label label-primary" data-url="${ctx}/admin/total/yestartdayshouru"
                         data-filter="false">
                        <span>昨日收入：</span>
                        <img src="${ctx}/static/images/loading.gif" style="height: 20px;width: 20px;"/>
                        <span></span>
                        <span>元</span>
                    </div>
                    <div class="countSum label label-primary" data-url="${ctx}/admin/total/todayshouru"
                         data-filter="false">
                        <span>今日收入：</span>
                        <img src="${ctx}/static/images/loading.gif" style="height: 20px;width: 20px;"/>
                        <span></span>
                        <span>元</span>
                    </div>
                </td>
            </tr>
        </table>
    </div>
    <div class="table-responsive" style="overflow:auto;">
        <table id="order-list" cellspacing="0" class="hover table table-striped table-bordered"
               width="200%">
            <thead>
            <tr>
                <th>公司</th>
                <th>状态</th>
                <th>流水号</th>
                <th>金额</th>
                <th>时间</th>
                <th>备注</th>
            </tr>
            </thead>
        </table>
    </div>
</div>
</#macro>
<#macro javascript>
<script id="action_tpl" type="text/html">
    {{#  if(d.audit_status==null){ }}
    <button type="button" class="btn btn-success btn-xs audit" id="{{d.id}}">审核</button>
    {{#  } }}
</script>
<script id="amount_tpl" type="text/x-handlebars-template">
    <form id="amount_form" action="" autocomplete="off">
        <table style="border-collapse:separate;border-spacing:10px;">
            <tr>
                <td>
                    <label for="amount" class="col-sm-4 control-label">金额:</label>
                </td>
                <td>
                    <div class="col-sm-7">
                        <input type="text" name="amount"
                               placeholder="如：500">
                    </div>
                </td>
            </tr>
        </table>
    </form>
</script>
<script type="text/javascript" src="${ctx}/static/js/app/admin/finance.js?_${.now?string("hhmmss")}"></script>
</#macro>