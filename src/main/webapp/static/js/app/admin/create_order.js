...
var url = {
    ...
    create: g.ctx + '/admin/order/createbyadmin'
}
    ...
    clickF: function (e) {
        ...
        geocoder.getAddress(lnglatXY, function (status, result) {
            if (status === 'complete' && result.info === 'OK') {
                ...
                var address = result.regeocode.pois[0].name.toString();
                dataInfo['address'] = address;
                console.log(address);
                var new_addressa = address.split("市");
                if (start) {
                    $("#reservation_address").val(new_addressa[new_addressa.length - 1] == null ? address : new_addressa[new_addressa.length - 1]);
                } else {
                    $("#destination").val(new_addressa[new_addressa.length - 1] == null ? address : new_addressa[new_addressa.length - 1]);
                }
            }
        });
    },
